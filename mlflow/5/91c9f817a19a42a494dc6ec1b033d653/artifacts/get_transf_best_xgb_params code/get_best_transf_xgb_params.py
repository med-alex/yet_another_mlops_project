...
mlflow.set_experiment("get_best_transf_xgb_transf_params")

with open(Path.cwd()/'data'/'transf_train_embeddings.pkl', 'rb') as file:
  train_embeddings = pickle.load(file)
Y_train = pd.read_csv(Path.cwd()/'data'/'transf_Y_train.csv', converters={'text': pd.eval})
...
with mlflow.start_run():
    ...
    mlflow.xgboost.log_model(best_model,
                             artifact_path='best_transf_xgboost',
                             registered_model_name='best_transf_xgboost')
    mlflow.log_artifact(local_path=Path.cwd()/'scripts'/'get_best_transf_xgb_params.py',
                        artifact_path='get_transf_best_xgb_params code')
