...
mlflow.set_experiment("test_tf_idf_model")

with open(Path.cwd()/'data'/'tf_idf_test_embeddings.pkl', 'rb') as file:
  test_embeddings = pickle.load(file)
Y_test = pd.read_csv(Path.cwd()/'data'/'tf_idf_Y_test.csv')

client = MlflowClient()
exp = client.get_experiment_by_name('get_best_tf_idf_xgb_params')
runs_info = client.search_runs(exp.experiment_id)

clf_model = mlflow.xgboost.load_model(f"runs:/{runs_info[0].info.run_id}/best_tf_idf_xgboost")
...
with mlflow.start_run():
    mlflow.log_input(mlflow.data.from_numpy(test_embeddings.toarray(), 
